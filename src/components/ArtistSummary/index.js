...
function ArtistSummary({name, albumCount, albumArtwork}) {
    ...
    return (
        <Link to={"/artists/" + name} className="col-12 col-lg-6 mb-4 text-decoration-none">
            <div className="rounded lightest-blue-hover light-blue p-3 text-light">
                <div className="justify-content-start mb-2 d-flex">
                    {albumArtwork.map(artwork => <img key={artwork} src={artwork} className="rounded w-25 me-2" alt="Album artwork" />)}
                </div>
